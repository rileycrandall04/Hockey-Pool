// Rule-tuning sweep: cut champion bonus, count best-3-of-4, and boost group-
// stage scoring — and see how each affects the spread among 12 owners and how
// often the champion's owner runs away with the pool. Parameterized scorer;
// production scoring untouched.
// Run: node --experimental-strip-types world-cup/scripts/simulate-tuning.ts
...
function ko(a: number, b: number, stage: string, rng: () => number): M { let [ga, gb] = simScore(a, b, rng); let s = false, pa: number | null = null, pb: number | null = null; if (ga === gb) { s = true; if (rng() < 0.5 + 0.15 * (rating(a) - rating(b))) { pa = 4; pb = 3; } else { pa = 3; pb = 4; } } return { stage, status: "final", home_country_id: a, away_country_id: b, home_goals: ga, away_goals: gb, went_to_shootout: s, home_pens: pa, away_pens: pb, _week: WEEK[stage] }; }
...
function simulateTournament(seed: number): M[] {
  ...
  for (const grp of groups) for (const [i, j, md] of sched) { const [hg, ag] = simScore(grp[i], grp[j], rng); const sh = st.get(grp[i])!, sa = st.get(grp[j])!; sh.gf += hg; sh.ga += ag; sa.gf += ag; sa.ga += hg; if (hg > ag) sh.p += 3; else if (hg < ag) sa.p += 3; else { sh.p++; sa.p++; } matches.push({ stage: "group", status: "final", home_country_id: grp[i], away_country_id: grp[j], home_goals: hg, away_goals: ag, went_to_shootout: false, home_pens: null, away_pens: null, _week: WEEK[`groupMD${md}`] }); }
...
function draft(seed: number): number[][] {
  ...
  const rosters: number[][] = Array.from({ length: 12 }, () => []); const pool = TEAMS.map((t) => t.id).sort((a, b) => fifaRank(a) - fifaRank(b)); let pick = 0;
  for (let r = 0; r < 4; r++) { const seq = r % 2 === 0 ? order : [...order].reverse(); for (const o of seq) rosters[o].push(pool[pick++]); }
  return rosters;
}

// ---- parameterized scorer with stage-aware values + best-N -----------------
type SV = { win: number; draw: number; goal: number; cs: number };
type Cfg = { group: SV; ko: SV; upset: number; champion: number; adv: Record<string, number>; bestN: number };
function scoreCountry(cid: number, matches: M[], cfg: Cfg): number {
  ...
  for (const m of matches) {
    ...
    const sv = m.stage === "group" ? cfg.group : cfg.ko;
    pts += gf * sv.goal - ga * 0.5;
    if (ga === 0) pts += sv.cs;
    if (m.went_to_shootout) { pts += cfg.ko.draw; const myP = (isHome ? m.home_pens : m.away_pens) ?? 0, opP = (isHome ? m.away_pens : m.home_pens) ?? 0; pts += myP > opP ? 4 : 2; if (m.stage === "final" && myP > opP) pts += cfg.champion; }
    else if (gf > ga) { pts += sv.win; if (m.stage === "group" && fifaRank(cid) > fifaRank(opp)) pts += cfg.upset; if (m.stage === "final") pts += cfg.champion; }
    else if (gf === ga) pts += sv.draw;
  }
...
const base: SV = { win: 3, draw: 1, goal: 1, cs: 1 };
const CONFIGS: Array<{ name: string; cfg: Cfg }> = [
  { name: "Baseline (champ 8, all 4 count)", cfg: { group: base, ko: base, upset: 5, champion: 8, adv: ADV, bestN: 4 } },
  { name: "Champ 4 + best-3-of-4", cfg: { group: base, ko: base, upset: 5, champion: 4, adv: ADV, bestN: 3 } },
  { name: "  + group goals x2", cfg: { group: { win: 3, draw: 1, goal: 2, cs: 1 }, ko: base, upset: 5, champion: 4, adv: ADV, bestN: 3 } },
  { name: "  + group fully boosted", cfg: { group: { win: 5, draw: 2, goal: 2, cs: 2 }, ko: base, upset: 5, champion: 4, adv: ADV, bestN: 3 } },
];

const N = 3000;
console.log(`\nAggregate over ${N} tournaments (12 owners x 4 teams). Smaller gap/stdev = tighter race.\n`);
console.log("Variant                              | winner | last | ratio | gap | stdev | champ-owner-wins");
console.log("-".repeat(104));
for (const { name, cfg } of CONFIGS) {
  let win = 0, last = 0, gap = 0, sd = 0, champWins = 0;
  for (let s = 0; s < N; s++) {
    const matches = simulateTournament(1000 + s); const rosters = draft(1000 + s);
    const finals = ownerTotals(rosters, matches, cfg); const stt = stats(finals);
    win += stt.max; last += stt.min; gap += stt.range; sd += stt.sd;
    const finalM = matches.find((m) => m.stage === "final")!; const champ = koWinner(finalM);
    const champOwner = rosters.findIndex((r) => r.includes(champ)); const poolWinner = finals.indexOf(Math.max(...finals));
    if (champOwner === poolWinner) champWins++;
  }
  const ratio = win / last;
  console.log(`${name.padEnd(36)} | ${r1(win / N).toString().padStart(6)} | ${r1(last / N).toString().padStart(4)} | ${(r1(ratio) + "x").padStart(5)} | ${r1(gap / N).toString().padStart(4)} | ${r1(sd / N).toString().padStart(5)} | ${(r1((champWins / N) * 100) + "%").padStart(16)}`);
}
console.log("\nratio = leader / last-place points (lower = proportionally tighter field).");
console.log("champ-owner-wins = how often the title team's owner also wins the pool (lower = less runaway).\n");
