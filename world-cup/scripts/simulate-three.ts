// Search for the rules that MAXIMIZE the odds of exactly 3 owners being able
// to win going into the final. Sweeps champion bonus x SF/Final multiplier x
// third-place multiplier; reports the full live-contender distribution.
// Run: node --experimental-strip-types world-cup/scripts/simulate-three.ts
...
type Cfg = { champion: number; sfFinalMult: number; thirdMult: number };
const ADV: Record<string, number> = { group: 0, r32: 2, r16: 2, qf: 2, sf: 2, third: 0, final: 2 };
function multOf(stage: string, cfg: Cfg) { return stage === "sf" || stage === "final" ? cfg.sfFinalMult : stage === "third" ? cfg.thirdMult : 1; }
function scoreCountry(cid: number, matches: M[], cfg: Cfg): number {
  ...
  for (const m of matches) {
    ...
    let base = gf - ga * 0.5; if (ga === 0) base += 1;
    let champ = 0;
    ...
    else if (gf > ga) { base += 3; if (m.stage === "group" && fifaRank(cid) > fifaRank(opp)) base += 5; if (m.stage === "final") champ = cfg.champion; }
    else if (gf === ga) base += 1;
    pts += base * multOf(m.stage, cfg) + champ;
  }
...
for (const champion of [8, 12, 18, 25, 35, 50])
  for (const sfFinalMult of [1, 1.5, 2, 3])
    for (const thirdMult of [1, 2, 3, 4]) {
      const cfg = { champion, sfFinalMult, thirdMult };
...
console.log(`\nTop configs by P(exactly 3 contenders), over ${N} tournaments each:\n`);
console.log("champ | SF/Fin x | 3rd x |   =1   |   =2   |   =3   |   =4   |  5+");
console.log("-".repeat(74));
const pct = (k: number, d: number[]) => (r1((d[k] / N) * 100) + "%").padStart(6);
for (const { cfg, dist } of results.slice(0, 14))
  console.log(`${String(cfg.champion).padStart(5)} | ${String(cfg.sfFinalMult).padStart(8)} | ${String(cfg.thirdMult).padStart(5)} | ${pct(1, dist)} | ${pct(2, dist)} | ${pct(3, dist)} | ${pct(4, dist)} | ${pct(5, dist)}`);
console.log("");
