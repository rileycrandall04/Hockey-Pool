...
async function joinLeagueAction(formData: FormData) {
  ...
  // The user isn't a member yet, so RLS won't let them read the league by
  // join_code. Use the service client for the lookup + team creation; the
  // user.id is still verified from the cookie-bound client above.
  ...
  if (insertError) {
    const friendly = insertError.message.includes("teams_league_id_owner_id_key")
      ...
      : insertError.message;
    redirect(`/leagues/join?error=${encodeURIComponent(friendly)}`);
  }
          ...
          <CardContent>
            <form action={joinLeagueAction} className="space-y-4">
              <div className="space-y-1">
                ...
                <Input id="join_code" name="join_code" required placeholder="ABC234" className="uppercase" />
              </div>
              <div className="space-y-1">
                <Label htmlFor="team_name">Your team name</Label>
                <Input id="team_name" name="team_name" required placeholder="Samba Kings" />
              </div>
              {error && (
